...
A "Wrapped"-style yearly recap, gated to release **January 1, 2027** — before that date, tapping its "View" button in Settings does nothing at all (previously showed an error toast; toasts don't exist in this app anymore, and nothing replaced that specific piece of feedback). Lives as a modal (`#novaWrappedModal`), not a separate page, so it shares the same signed-in session's data as the rest of the app instead of loading a second copy of the Firebase SDK.
