...
Shown by default in the HTML itself, not toggled on by JS - the one thing on the page visible before any script has even run, covering the real gap between the page rendering and Firebase Auth resolving. Tied to genuine readiness (`hideSplashScreen()` in `app.js`, called once it's known whether to show sign-in or the main app), not a fixed delay - a fixed wait would make the app feel slower than it actually is on a fast connection, working against the whole reason the core scripts load with `defer` in the first place (see Load performance below). A small minimum keeps it from flashing in and out too quickly on a fast connection to register.

The 8-second safety net lives outside `app.js` entirely - a small standalone inline `<script>` right after `#splashScreen`'s own markup in `index.html`, not a function in the main app file. That's deliberate, not a style choice: a timer set up inside `app.js` can only ever run if `app.js` itself loaded, parsed, and executed successfully - exactly the scenario that needs covering is `app.js` failing for some reason, so the fallback can't depend on the thing that might be broken. It makes no assumptions about anything else on the page - just checks whether `#splashScreen` still exists in the DOM (the normal hide path removes it outright), and if so, swaps its content for an honest "this is taking longer than expected" message with a manual reload button, rather than ever leaving someone stuck with no way out.
