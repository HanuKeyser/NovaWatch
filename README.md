...
## Round: season selector "background block" - the trough itself was the problem

The previous round's fix (see directly above) wrapped `.season-selector` in a shared background "trough" - reasoning that individually-shadowed pills sitting adjacent were merging into a shadowed band that read as "in a block". That reasoning was sound for the specific symptom it was solving, but the fix itself introduced the exact same complaint through a different mechanism: a shared, visible background/border container *is* "a block" the buttons sit inside - confirmed directly from a follow-up screenshot showing Season 1/2/3 sitting inside a visibly separate grey rounded-rect strip.

**Reversed the trough, but not back to the original per-pill-shadow version that caused the first complaint.** `.season-selector` no longer has any background/border/padding of its own - it's fully transparent again, just a flex row. Each `.season-button` now carries its own individual `background: var(--surface-2)` and `border: 1px solid var(--glass-border-subtle)` - enough fill/edge to read as a distinct floating pill against the page - but deliberately **no `box-shadow`** this time. That's the part that actually caused the original merged-shadow-band problem; fill and border alone don't merge or stack the way shadows do, so this avoids both complaints at once rather than just trading one for the other. `.season-button.active` is unchanged (still its own blue gradient + shadow - a single active pill's shadow was never the issue, only several inactive ones stacked adjacently). Button height went from the trough's 36px back to 40px, matching every other standalone pill in the app (`.hero-pill-btn`, `.segmented-btn`), and the gap widened from the trough's tight 4px to 8px now that pills are separated by their own edges rather than sitting inside one container.

Ran the established validation sweep (JS syntax, CSS brace balance) - all clean. Bumped `service-worker.js`'s `CACHE_NAME` (`v21` → `v22`).
