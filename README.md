...
**Edge-to-edge page content** — extends the same idea from the nav bar to every tab's own content: `.page` carries no horizontal margin of its own anymore, so cards (`.hero-card`, `.analytics-card`, `.continue-card`, library posters) span the full width instead of floating with a visible strip of page background on either side - each relies on its own existing internal padding for content breathing room rather than an outer page margin providing it. The margin comes back, deliberately, only on the specific things that need it for readability or usability rather than visual weight: section titles, search bars, segmented toggles, and text-heavy list rows (Discover's search results) - a pure poster grid can go edge to edge without hurting anything, but a line of body text touching the physical screen edge is a real readability problem, not a stylistic one. Scoped entirely by CSS selector rather than classes added throughout the markup, so it's a single, easily reversible place to tune if this doesn't end up feeling right.
