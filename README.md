...
- **Tablet** (`min-width: 768px`) - not a separate tablet layout, same bottom nav/page structure/components everywhere, just enough widening that an iPad doesn't get a stretched phone screen with dead space down both sides. `.app`'s content column widens from 600px to 1024px (the 12.9" iPad Pro's own portrait width - covers every common tablet in portrait completely), the bottom nav widens to 620px to match without its 4 buttons looking sparse, and `.library-grid`/`.poster-picker-grid` gain a 4th column since images benefit from the extra width in a way running text doesn't.
- **Large screens** (`min-width: 1280px`) - a laptop/desktop window, or a large tablet in landscape (a 12.9" iPad Pro is ~1366px there). A further step past the tablet block, not a from-scratch desktop redesign - still the same centered mobile-first column, just with more breathing room (`.app` to 1140px, the nav bar to 680px) and a 5th poster-grid column now that there's room for it.
- **Large desktop monitors** (`min-width: 1600px`, then `min-width: 2000px`) - two further steps so a real external monitor doesn't just freeze at the 1280px block's 1140px column forever, with a growing strip of unused background on either side as the window gets wider. 1600px covers common laptop/external-monitor widths (`.app` to 1320px, 6 poster-grid columns); 2000px covers 4K/ultrawide monitors specifically (`.app` to 1480px, 7 columns). Capped there deliberately - a poster grid or a line of text stretched edge-to-edge on a 34" ultrawide reads as sparse, not better, so growing the column indefinitely with screen width would work against readability past a certain point. The `--bg-mesh` background (see tokens.css) already carries real color/light of its own, so the margin past this cap reads as deliberate framing rather than a layout that ran out of ideas.
