...
**Upcoming type filter (implemented):** added an All/TV Shows/Movies segmented filter above the Upcoming schedule (reusing the existing `.segmented-toggle.three-way` styling), rather than splitting Upcoming into separate TV/Movie tabs like Discover/Library — the combined, date-sorted schedule stays the default since that's the actual point of the tab (what's releasing when, across both types), the filter just lets you narrow it on request. `getUpcomingItems()` already accepted a `filterType` argument that wasn't exposed in the UI - `setUpcomingFilter()` in `app.js` just wires it up.

**Bottom nav visual rework (implemented):** pushed the nav bar further toward genuine "Liquid Glass" than the shared `--glass-chrome-*` tokens (which stay as-is for sheets/toasts/pill controls/circular buttons) - much lower fill opacity so background content actually shows through, stronger blur+saturation to keep it legible at that opacity, a full capsule shape instead of the app's usual card radius, a radial highlight near the top-left standing in for light catching curved glass, and a slow, low-contrast animated diagonal sheen (respects `prefers-reduced-motion`) so it reads as moving glass rather than static frosting. Scoped entirely to `.bottom-nav`/`.nav-indicator` in `app.css`.
