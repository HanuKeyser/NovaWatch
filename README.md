...
**Bottom navigation** — a floating "Liquid Glass" pill bar with 3 tabs (Home/Library/Upcoming), plus Search as its own standalone circular button to the left of it (`.nav-bar-row` wraps both, replacing the fixed positioning `.bottom-nav` used to own by itself). Both share the same visual chrome recipe so they read as one consistent piece split into two, not two different-looking buttons. Search moving out of the sliding pill needed zero changes to the pill's own mechanics: the drag-to-switch gesture and the indicator's positioning math both already worked purely from `nav.querySelectorAll(".nav-button")` and `getBoundingClientRect()` on whatever's actually inside `.bottom-nav` at the time, with no tab count hardcoded anywhere - and `showPage()`'s active-state toggling already looked buttons up by `id` and toggled `.active` via a document-wide selector, neither of which cared where in the DOM a button physically lived. An edge-to-edge, flush-to-the-bottom version of the main bar was tried and reverted after not looking right on real devices - back to the original floating pill, with margin on every side. Supports both tapping a tab and dragging a finger across the main bar to switch between its 3 tabs, with the pill indicator tracking the finger continuously during a drag and settling into place on release.
