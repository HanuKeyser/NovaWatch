...
**Top status-bar blur** — a fixed, blurred strip sitting exactly over the real device status-bar area, fading to transparent going down. `black-translucent` (the app's own status-bar style) is fully transparent by design, so without this, scrolling content passes directly underneath the clock/battery icons with zero visual transition at all - this makes it blur and dim as it scrolls past instead, the same way real native apps handle it. Purely decorative, never intercepts a touch, and collapses to zero height with nothing to do on any device with no real inset.
