...
1. **Only for a show positively confirmed as a real US broadcast or cable network** (NBC, ABC, CBS, FOX, The CW, PBS, plus ~30 cable networks like AMC, FX, USA, TNT, HBO, Showtime) — detected from TMDB's own network data — is a real per-episode airtime from TVmaze even considered. Everything else (a streaming platform, an unrecognized network, no network data at all) skips TVmaze entirely and uses the streaming convention below.
2. **Even then, that network match is double-checked against confirmed watch-provider data** (the same TMDB endpoint the "Watch On" section uses) before TVmaze is trusted. TMDB's `networks` field mainly tracks the *producing* network, not every platform a show is distributed on — a cable-produced show distributed internationally through a streaming app can list only the producing network, with the streaming platform never mentioned at all. If a known streaming platform (Netflix, Disney+, Hulu, Max, Apple TV+, Paramount+, Peacock, Amazon, Crunchyroll) is confirmed to actually carry the title in the viewer's own region, that overrides the network match and the streaming convention is used instead, regardless of what network produced it.
3. **The streaming convention** — midnight Pacific time, DST-aware — applies to everything that isn't a confirmed, un-overridden broadcast/cable match: every major streaming platform, anything unrecognized, and any broadcast/cable match that step 2 overrode.
4. **Broadcast/cable shows that survive both checks** fall back to 8pm Eastern (DST-aware) if TVmaze itself doesn't have data for that specific episode.

Movies always use the streaming/Pacific convention, since TVmaze doesn't cover them and the broadcast/cable concept doesn't apply. Whichever source ends up being used is what actually gates when something leaves Upcoming and starts counting as watchable elsewhere in the app (Continue Watching, marking episodes watched, etc.), not just a rough "today" check. Newly-computed data replaces stale values automatically on the next background sync or the next time the show's own details are opened; visiting the Upcoming tab also triggers a fresh sync if one hasn't run in the last couple of minutes, so switching to it is a reliable way to get current data without waiting.
