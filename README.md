...
**Bottom nav visual rework (implemented, revised):** pushed further toward genuine "Liquid Glass" across two rounds - very low fill opacity so background content actually shows through, strong blur+saturation to stay legible at that opacity, a full capsule shape instead of the app's usual card radius, a top-left radial highlight standing in for light catching curved glass, a slow low-contrast animated diagonal sheen (respects `prefers-reduced-motion`), no border (edge definition comes entirely from box-shadow rim highlights now), and refined nav buttons - thinner icon strokes, tighter label tracking, a soft colored glow behind the active icon instead of a flat color swap. Scoped to `.bottom-nav`/`.nav-indicator`/`.nav-button` in `app.css`.
