...
**Edge-to-edge page content** — extends the same idea from the nav bar to every tab's own content, but tuned by element rather than one blanket rule: `.page`'s own horizontal margin dropped from 18px to a much tighter 12px (not zero) for cards (`.hero-card`, `.analytics-card`, `.continue-card`) and text, since a literal zero margin made rounded corners, borders, and shadows look like a mistake rather than a style choice once they were touching the flush screen edge with nothing around them - a shape needs a little room for its own rounding, border, and shadow to read as intentional. Poster grids (`.library-grid`, used by both Library and Upcoming) get true zero-margin edge-to-edge instead, via a deliberate negative-margin bleed past that 12px - pure image content doesn't have the same problem a card with a visible border and all-around glow does, and a poster's own corner radius is small enough relative to its size to read fine flush against the true edge. Scoped entirely by CSS, so it's a single, easily reversible place to tune further if it still doesn't feel right.
